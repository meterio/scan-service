...
export class MetricCMD extends CMD {
  ...
  private async updatePowInfo() {
    ...
    this.log.info(`btcPrice: ${btcPrice}, btcHashrate: ${btcHashrate}`);
    const rewardPerDay = new BigNumber(efficiency).dividedBy(10).times(24);
    this.log.info(`rewardPerDay: ${rewardPerDay}`);
    const costParity = new BigNumber(3.125) // bitcoin reward
      .times(24 * 6)
      .times(1000)
      .times(btcPrice)
  ...
  private async updateAddressCount() {
  ...
  private async checkRpc(url: string) {
    let epoch = 0;
    let number = 0;
    try {
  ...
  private async alertIfNetworkHalt() {
  ...
  private async updateValidatorRewards() {
  ...
  private async updatePowHashrate() {
  ...
  private async updateMarketPriceWithCoingecko() {
  ...
  private async updateMarketPriceWithCoinMarketCap() {
    try {
  ...
  private async updateTxMaxCount() {
  ...
  private async updateAuctionInfo() {
  ...
  private async updateInvalidNodes() {
  ...
  private async updateSlashingInfo() {
  ...
  private async updateStakingInfo() {
  ...
  private async updateCirculationAndRank() {
    // Update circulation
  ...
  private async updateVerifiedContracts() {
    ...
    for (const c of unverified) {
      ...
      const creationMatches = await this.contractRepo.findUnverifiedContractsWithCreationInputHash(c.creationInputHash);
  ...
  private async adjustTotalSupply() {
    const contracts = await this.contractRepo.findByType(ContractType.ERC20);
    this.log.info(`start adjusting totalsupply for ${contracts.length} contracts...`);
    let updateCount = 0;
    ...
    this.log.info(`done adjusting totalsupply for ${updateCount} contracts`);
  }
    ...
    for (;;) {
      try {
        ...
        if (config.powEnabled) {
          ...
          if (shouldRun(index, every5m)) {
            await this.updatePowInfo();
          }

          // update bitcoin info every 5 minutes
          if (shouldRun(index, every20m)) {
            await this.updatePowHashrate();
          }
        }

        if (shouldRun(index, every6h)) {
          await this.updateAddressCount();
        }

        // update pos best, kblock & seq
        await this.updatePosInfo();

        // update token rank
        if (shouldRun(index, every10m)) {
          await this.updateTokenRank();
        }

        // check network, if halt for 2 mins, send alert
        if (shouldRun(index, every2m)) {
          await this.alertIfNetworkHalt();
        }

        if (this.network == Network.MainNet) {
          if (shouldRun(index, every10m)) {
            await this.checkRpc('http://rpc-asia.meter.io');
            await this.checkRpc('http://rpc-eu.meter.io');
            await this.checkRpc('http://rpc-us.meter.io');
          }
        }

        // update price/change every 10 minutes
        if (shouldRun(index, every30m)) {
          await this.updateMarketPriceWithCoingecko();
        }

        // update circulation
        if (shouldRun(index, every4h)) {
          await this.updateCirculationAndRank();
        }
        ...
        if (shouldRun(index, every5m)) {
          ...
          await this.updateSlashingInfo();

          // update network status
          await this.updateInvalidNodes();
        }

        // update auction info
        if (config.auctionEnabled && shouldRun(index, every5m)) {
          await this.updateAuctionInfo();
        }

        if (shouldRun(index, every1h)) {
          await this.updateTxMaxCount();
        }
        ...
        // adjust total supply
        if (shouldRun(index, every2h)) {
          await this.adjustTotalSupply();
        }
      } catch (e) {
