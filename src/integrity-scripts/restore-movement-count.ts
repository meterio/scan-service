...
const runAsync = async (options) => {
  ...
  for (let i = 0; i < best; i += step) {
    ...
    const txs = await txRepo.findEmptyMovementsInBlockRangeSortAsc(start, end);
    if (txs.length < 20) {
      await PromisePool.withConcurrency(20)
        .for(txs)
        .process(async (tx) => {
          const movementCount = await movementRepo.countByTxHash(tx.hash);
          if (tx.movementCount != movementCount) {
            tx.movementCount = movementCount;
            await tx.save();
            console.log(`update movement count to ${movementCount} for tx ${tx.hash}`);
          }
        });
    } else {
